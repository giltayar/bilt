...
describe('artifactWalker', function() {
  ...
  it('should work with just one dir', async function() {
    ...
    const fetchEntriesOfDir = sinon
      ...
      .withArgs(dir)
      .returns(
        Promise.resolve([{name: 'zoo.json', type: 'file'}, {name: 'package.json', type: 'file'}]),
      )
    ...
    const deps = await artifactWalker(fetchEntriesOfDir, dir, extractArtifacts, id)
  ...
  it('should work with two subdirs', async function() {
    ...
    fetchEntriesOfDir
      .withArgs(dir)
      .returns(Promise.resolve([{name: 'a', type: 'dir'}, {name: 'b', type: 'dir'}]))
    fetchEntriesOfDir
      .withArgs(dir + '/a')
      .returns(
        Promise.resolve([{name: 'package.json', type: 'file'}, {name: 'a.json', type: 'file'}]),
      )
    fetchEntriesOfDir
      .withArgs(dir + '/b')
      .returns(
        Promise.resolve([{name: 'package.json', type: 'file'}, {name: 'b.json', type: 'file'}]),
      )
    const extractArtifacts = sinon.stub()
  ...
  it('should work with subdir inside another', async function() {
    const dir = 'foo'
    const fetchEntriesOfDir = sinon.stub()
    fetchEntriesOfDir.withArgs(dir).returns(Promise.resolve([{name: 'a', type: 'dir'}]))
    fetchEntriesOfDir
      .withArgs(dir + '/a')
      .returns(Promise.resolve([{name: 'a.json', type: 'file'}, {name: 'b', type: 'dir'}]))
    fetchEntriesOfDir
      .withArgs(dir + '/a/b')
      .returns(Promise.resolve([{name: 'package.json', type: 'file'}]))
    const extractArtifacts = sinon.stub()
  ...
  it('should support two artifacts in a dir and combine them with extractorMerger', async function() {
    ...
    const deps = await artifactWalker(fetchEntriesOfDir, dir, extractArtifacts, artifacts => {
