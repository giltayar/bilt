...
describe('artifactWalker', function() {
  ...
  it('should work with just one dir', async function() {
    const dir = 'foo'
    const fetchEntriesOfDir = sinon.stub().withArgs(dir, 'ignore').returns(
      Promise.resolve({
        entries: [{name: 'zoo.json', type: 'file'}, {name: 'package.json', type: 'file'}],
      }),
    )
  ...
  it('should work with two subdirs', async function() {
    ...
    fetchEntriesOfDir.withArgs(dir, 'ignore').returns(
      Promise.resolve({
        entries: [{name: 'a', type: 'dir'}, {name: 'b', type: 'dir'}],
        ignoreStack: 'ignore',
      }),
    )
    fetchEntriesOfDir.withArgs(dir + '/a', 'ignore').returns(
      Promise.resolve({
        entries: [{name: 'package.json', type: 'file'}, {name: 'a.json', type: 'file'}],
        ignoreStack: 'ignore',
      }),
    )
    fetchEntriesOfDir.withArgs(dir + '/b', 'ignore').returns(
      Promise.resolve({
        entries: [{name: 'package.json', type: 'file'}, {name: 'b.json', type: 'file'}],
        ignoreStack: 'ignore',
      }),
    )
    const extractArtifacts = sinon.stub()
  ...
  it('should work with subdir inside another', async function() {
    ...
    fetchEntriesOfDir.withArgs(dir + '/a', 'ignore').returns(
      Promise.resolve({
        entries: [{name: 'a.json', type: 'file'}, {name: 'b', type: 'dir'}],
        ignoreStack: 'ignore',
      }),
    )
    fetchEntriesOfDir
