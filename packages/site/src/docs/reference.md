...
* `jobs`: either the JSON of a build configuration (that builds the `packages`), or a string
  ...
  Bilt](../build-configurations#the-default-build-configuration).
* `jobDefaults`: an object whose keys are `jobIds` (see [build
   configuration](../build-configurations)), and values are objects that define
...
1. The configuration file's `buildConfiguration` field determines what the [build
   configuration](../build-configurations) is. The build configuration determines
...
1. `bilt` builds each package according to the [build
   configuration](../build-configurations). The build configuration also includes steps that
...
1. As described [elsewhere](../how-bilt-works#packages-built-how),
...
See [here](../build-configurations).

### Default build steps

You can find the build configuration for the default build configuration
[here](https://github.com/giltayar/bilt/blob/master/packages/types/src/types.ts). For a human discussion
